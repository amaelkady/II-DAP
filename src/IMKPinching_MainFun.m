function [fi,ki, Energy,Backbone_j_1,Flag,u0, du_i_1] = IMKPinching_MainFun(Backbone,Energy,Backbone_j_1,Flag,u0,ui, ui_1, du_i_1,fi_1)
%#####################################################################################################################################################################
...
if Flag.Failure ~= 1
    ...
    elseif fi_1<0 &&  ui_1 <= Backbone_j_1.Upeak_neg
    ...
    if Flag.Excursion==1
        if du>0
            Backbone_j_1.Upl = Backbone_j_1.Upeak_pos - (Backbone_j_1.Fpeak_pos/Backbone_j_1.Kul);
            Backbone_j_1.Ubp = (1-Backbone.kappaD) * Backbone_j_1.Upl ;
            Backbone_j_1.Fbp = Backbone.kappaF * Backbone_j_1.Fpeak_pos * abs((Backbone_j_1.Ubp -u0)/(Backbone_j_1.Upeak_pos -u0));
        elseif du<0
            Backbone_j_1.Upl = Backbone_j_1.Upeak_neg - (Backbone_j_1.Fpeak_neg/Backbone_j_1.Kul);
            Backbone_j_1.Ubp = (1-Backbone.kappaD) * Backbone_j_1.Upl ;
            Backbone_j_1.Fbp = Backbone.kappaF * Backbone_j_1.Fpeak_neg * abs((Backbone_j_1.Ubp -u0)/(Backbone_j_1.Upeak_neg -u0));
        end
    end
    ...
    if fi_1 + du*Backbone_j_1.Kul >= 0
        ...
        elseif Flag.Excursion==1
            if Flag.TargetPeak==0 && u0>=Backbone_j_1.Ubp
                Backbone_j_1.Krel  =  Backbone_j_1.Fpeak_pos / (Backbone_j_1.Upeak_pos - u0);
            elseif Flag.TargetPeak==0 && u0<=Backbone_j_1.Ubp
                Backbone_j_1.Krel  =  Backbone_j_1.Fbp / (Backbone_j_1.Ubp - u0);
            elseif Flag.TargetPeak==1
        ...
        elseif Flag.Reload==1 && ui >= Backbone_j_1.ULastPeak_pos && ui <= Backbone_j_1.Upeak_pos && Backbone_j_1.FLastPeak_pos <= Backbone_j_1.Fpeak_pos
            if Flag.TargetPeak==1
                if Backbone_j_1.FLastPeak_pos <= Backbone_j_1.Fbp && ui <= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fbp-Backbone_j_1.FLastPeak_pos)/(Backbone_j_1.Ubp-Backbone_j_1.ULastPeak_pos);
                elseif Backbone_j_1.FLastPeak_pos <= Backbone_j_1.Fbp && ui >= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fpeak_pos-Backbone_j_1.Fbp)/(Backbone_j_1.Upeak_pos-Backbone_j_1.Ubp);
                else
        ...
        elseif du >= 0 && ui <= Backbone_j_1.Upeak_pos
            if Flag.TargetPeak==0 && ui>=Backbone_j_1.Ubp
                Backbone_j_1.Krel  =  (Backbone_j_1.Fpeak_pos - fi_1) / (Backbone_j_1.Upeak_pos - ui_1);
            elseif Flag.TargetPeak==0 && ui<=Backbone_j_1.Ubp
                Backbone_j_1.Krel  = (Backbone_j_1.Fbp) / (Backbone_j_1.Ubp - u0);
            elseif Flag.TargetPeak==1 && ui<=Backbone_j_1.ULastPeak_pos
                %Backbone_j_1.Krel  = (Backbone_j_1.FLastPeak_pos) / (Backbone_j_1.ULastPeak_pos - u0);
                if Backbone_j_1.Ubp <= Backbone_j_1.ULastPeak_pos && ui <= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fbp) / (Backbone_j_1.Ubp - u0);
                elseif Backbone_j_1.Ubp <= Backbone_j_1.ULastPeak_pos && ui >= Backbone_j_1.Ubp
    ...
    if fi_1 + du*Backbone_j_1.Kul <= 0
        ...
        elseif Flag.Reload==1 && ui <= Backbone_j_1.ULastPeak_neg && ui >= Backbone_j_1.Upeak_neg && Backbone_j_1.FLastPeak_neg >= Backbone_j_1.Fpeak_neg
            if Flag.TargetPeak==1 && ui<=Backbone_j_1.ULastPeak_neg
                if Backbone_j_1.FLastPeak_neg >= Backbone_j_1.Fbp && ui <= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fbp-Backbone_j_1.FLastPeak_neg)/(Backbone_j_1.Ubp-Backbone_j_1.ULastPeak_neg);
                elseif Backbone_j_1.FLastPeak_neg >= Backbone_j_1.Fbp && ui <= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fpeak_neg-Backbone_j_1.Fbp)/(Backbone_j_1.Upeak_neg-Backbone_j_1.Ubp);
                else
            ...
            % CASE 4: WHEN LOADING IN GENERAL TOWARDS THE TARGET PEAK
        elseif du <= 0 && ui >= Backbone_j_1.Upeak_neg
            if Flag.TargetPeak==0 && ui<=Backbone_j_1.Ubp
                Backbone_j_1.Krel  =  (Backbone_j_1.Fpeak_neg - fi_1) / (Backbone_j_1.Upeak_neg - ui_1);
            elseif Flag.TargetPeak==0 && ui>=Backbone_j_1.Ubp
                Backbone_j_1.Krel  =  (Backbone_j_1.Fbp) / (Backbone_j_1.Ubp - u0);
            elseif Flag.TargetPeak==1&& ui>=Backbone_j_1.ULastPeak_neg
                %Backbone_j_1.Krel  = (Backbone_j_1.FLastPeak_neg) / (Backbone_j_1.ULastPeak_neg - u0);
                if Backbone_j_1.Ubp >= Backbone_j_1.ULastPeak_neg && ui >= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.Fbp) / (Backbone_j_1.Ubp - u0);
                elseif Backbone_j_1.Ubp >= Backbone_j_1.ULastPeak_neg && ui <= Backbone_j_1.Ubp
                    Backbone_j_1.Krel  = (Backbone_j_1.FLastPeak_neg -fi_1) / (Backbone_j_1.ULastPeak_neg - ui_1);
                else
                    Backbone_j_1.Krel  = (Backbone_j_1.FLastPeak_neg) / (Backbone_j_1.ULastPeak_neg - u0);
                end
            elseif Flag.TargetPeak==1 && ui<=Backbone_j_1.ULastPeak_neg
