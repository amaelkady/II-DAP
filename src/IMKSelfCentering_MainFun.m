...
function [Ri, Mi, Di, Di_1, BackbonePos_j_1, BackboneNeg_j_1, Beta_j_1, Energy_Excrsni_1, Energy_Excrsn, Energy_Rev, Energy_total, Flags, InitialValues, Smoothparamters, TangentK]=IMKSmooth_MainFunCC (Ri, Ri_1, Mi_1, Di_1, BackbonePos_Original, BackboneNeg_Original, BackbonePos_j_1, BackboneNeg_j_1 , Beta_j_1, Energy_Excrsni_1, Energy_Excrsn, Energy_Rev, Energy_total, Flags, Constants, InitialValues, Smoothparamters)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   Rmax_pos0 = BackbonePos_Original(3);
   Kpc_pos0 = BackbonePos_Original(5);
        Fy_pos0 = BackbonePos_Original(6);
        Fmax_pos0 = BackbonePos_Original(8);
         Fres_pos0 = BackbonePos_Original(11);
         Fll_pos0 = BackbonePos_Original(12);
         Rll_pos0 = BackbonePos_Original(13);
         Kul_pos0 = BackbonePos_Original(14);
    if Fres_pos0<0.0; Fres_pos0=0; end
         
   Rmax_neg0 = BackboneNeg_Original(3);
   Kpc_neg0 = BackboneNeg_Original(5);
        Fy_neg0 = BackboneNeg_Original(6);
        Fmax_neg0 = BackboneNeg_Original(8);
         Fres_neg0 = BackboneNeg_Original(11);
         Fll_neg0 = BackboneNeg_Original(12);
         Rll_neg0 = BackboneNeg_Original(13);
        Kul_neg0 = BackboneNeg_Original(14);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Check for Fail Flag
    if Ri>=Uu_pos || Ri>=(Rmax_pos0+Fmax_pos0/Kpc_pos0) 
        Fail_FlagPos=1;
    end
    if Ri<=-Uu_neg || Ri<=-(Rmax_neg0+Fmax_neg0/Kpc_neg0) 
        Fail_FlagNeg=1;
    end
...
%% Self Centering Boundary

% Calcaulte the intersection rotation between the post-capping slope and
% the unloading slope
b1 = Fmaxi - Kpci*Umaxi;
b2 = Fll_pos0-Kul_pos0*Rll_pos0;
RintrsctSC = -b1 / Ki;
Rintrsct = -(b2 - b1) / (Kp_x - Ki);

a1_pos=Fmax_pos0+Kpc_pos0*Rmax_pos0;
a2_pos=Fll_pos0-Kul_pos0*Rll_pos0;
Rintersct_pos0=(a1_pos-a2_pos)/(Kul_pos0+Kpc_pos0);

a1_neg=Fmax_neg0+Kpc_neg0*Rmax_neg0;
a2_neg=Fll_neg0-Kul_neg0*Rll_neg0;
Rintersct_neg0=(a1_neg-a2_neg)/(Kul_neg0+Kpc_neg0);

if Ri> 0 && Ri>  Rll_pos0 && Mi_boundary_Neg< Fll_pos0+Kul_pos0*(    Ri -Rll_pos0); Mi_boundary_Neg= Fll_pos0+Kul_pos0*(    Ri -Rll_pos0); end
if Ri<=0 && Ri< -Rll_neg0 && Mi_boundary_Pos>-Fll_neg0-Kul_neg0*(abs(Ri)-Rll_neg0); Mi_boundary_Pos=-Fll_neg0-Kul_neg0*(abs(Ri)-Rll_neg0); end

if Ri>0 && Di<0.0
    %if Mi_boundary_Neg< Fll_pos0+Kul_pos0*(    Ri -Rll_pos0); Mi_boundary_Neg= Fll_pos0+Kul_pos0*(    Ri -Rll_pos0); end
    if Ri<Rll_pos0
        Mi_boundary_Pos=Ke*Ri;
        Mi_boundary_Neg=Ke*Ri;
    end
elseif Ri<0 && Di>0.0
    %if Mi_boundary_Pos>-Fll_neg0-Kul_neg0*(abs(Ri)-Rll_neg0); Mi_boundary_Pos=-Fll_neg0-Kul_neg0*(abs(Ri)-Rll_neg0); end
    if abs(Ri)<Rll_neg0
        Mi_boundary_Neg=Ke*Ri;
        Mi_boundary_Pos=Ke*Ri;
    end
    ...
    if Ri> 0 && Ri>  Rintersct_pos0
    Mi =Fmax_pos0-Kpc_pos0*(Ri-Rmax_pos0);
    end
    if Ri< 0 && Ri<  -Rintersct_neg0
        Mi =-(Fmax_neg0-Kpc_neg0*(abs(Ri)-Rmax_neg0));
    end
